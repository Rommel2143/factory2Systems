...
Public Class setplan_ADF
    ...
    Private Sub cmb_partcode_MouseClick(sender As Object, e As MouseEventArgs) Handles cmb_partcode.MouseClick
        cmb_display("SELECT CONCAT(partcode,' | ', partname) FROM `assy_masterlist` WHERE location='" & lineLocation & "' GROUP BY partcode", cmb_partcode)
    End Sub
    ...
    Private Sub setplan_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        cmb_display("SELECT line FROM prod_PCline WHERE location='" & lineLocation & "' GROUP BY line", cmb_line)
    End Sub
